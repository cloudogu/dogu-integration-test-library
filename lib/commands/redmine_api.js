...
const redmineDeleteUser = (username, exitOnFail = true) => {
    return env.GetAdminCredentials().then(({AdminUsername, AdminPassword}) => {
        return cy.redmineGetCurrentUserJsonWithBasic(AdminUsername, AdminPassword, exitOnFail).then((responseUserJson) => {
            if (responseUserJson.status != 200) {
                return
            }
            return cy.redmineGetUsersJson(responseUserJson.body.user.api_key, exitOnFail).then((response) => {
