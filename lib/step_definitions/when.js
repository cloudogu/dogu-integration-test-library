...
module.exports.register = function () {
    ...
    When(/^the user types in wrong login credentials$/, function () {
        cy.get('input[data-testid="login-username-input-field"]').type("RaNd0mUSR_?123")
        cy.get('input[data-testid="login-username-password-field"]').type("RaNd0mPWöäü_?123")
    });

    When(/^the user types in correct login credentials$/, function () {
        cy.fixture("testuser_data").then(function (testUser) {
            cy.get('input[data-testid="login-username-input-field"]').type(testUser.username)
            cy.get('input[data-testid="login-username-password-field"]').type(testUser.password)
        })
    });
