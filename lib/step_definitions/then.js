...
module.exports.register = function () {
    ...
    Then(/^the login page informs the user about invalid credentials$/, function () {
        cy.get('#loginErrorsPanel > p').contains("Authentication attempt has failed, likely due to invalid credentials. Please verify and try again.")
    });

    Then(/^the login page informs the user about not recognizing the given user account$/, function () {
        cy.get('#loginErrorsPanel > p').contains("Your account is not recognized and cannot login at this time.")
    });
}