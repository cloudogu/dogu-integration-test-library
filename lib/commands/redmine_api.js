...
const redmineDeleteUser = (username, exitOnFail = true) => {
    cy.redmineGetCurrentUserJsonWithBasic(env.GetAdminUsername(), env.GetAdminPassword(), exitOnFail).then((responseApiKey) => {
        expect(responseApiKey.status).to.eq(200)
        cy.redmineGetUsersJson(responseApiKey.body.user.api_key, exitOnFail).then((response) => {
