 ...
 * @param {boolean} pwdReset - Indicates whether the user must change his password at the next login.
 */
const usermgtCreateUser = (username, givenName, surname, displayName, mail, password, pwdReset = false, groups = null) => {
    cy.request({
        method: "POST",
        url: Cypress.config().baseUrl + "/usermgt/api/users/",
        followRedirect: !env.IsMultinode(),
        auth: {
            'user': env.GetAdminUsername(),
            'pass': env.GetAdminPassword()
        },
        ...
        if (response.status === 302) {
            cy.log("Redirect to: " + response.headers.location)
        }
        expect(response.status).to.eq(env.IsMultinode() ? 200 : 201)
        if (groups) {
...
/**
 * Returns the HTTP status to indicate if the group by the given name exists (HTTP 200) or not (HTTP 404).
  * @param {String} name - The name of the group that should be retrieved.
 */
const usermgtGetGroupWithStatus = (name)  => {
    cy.request({
        method: "GET",
        url: Cypress.config().baseUrl + "/usermgt/api/groups/" + name,
        followRedirect: false,
...
module.exports.register = function () {
    ...
    Cypress.Commands.add("usermgtGetGroupWithStatus", usermgtGetGroupWithStatus)
